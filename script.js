...
// API 常量
const ARK_BASE_URL = 'http://localhost:3000/api/v3'; // 使用本地代理服务器
...
const LLM_SYSTEM_PROMPT = [
    ...
    '}',
    '',
    '重要：请严格按照上述JSON格式输出，不要添加任何其他文字说明，确保输出的是有效的JSON格式。'
].join('\n');

// 工具函数
function extractTextFromMessageContent(content) {
    if (!content) {
        console.log('extractTextFromMessageContent: content is empty');
        return '';
    }
    
    console.log('extractTextFromMessageContent: 原始content:', content);
    
    if (typeof content === 'string') {
        console.log('extractTextFromMessageContent: content是字符串，直接返回');
        return content;
    }
    
    if (Array.isArray(content)) {
        console.log('extractTextFromMessageContent: content是数组，处理每个部分');
        const textParts = content.map(part => {
            if (typeof part?.text === 'string') {
                ...
                console.log('extractTextFromMessageContent: 跳过无效部分:', part);
                return '';
            }
        }).filter(text => text.length > 0);
        
        const result = textParts.join('\n');
        console.log('extractTextFromMessageContent: 合并后的文本:', result);
        return result;
    }
    
    console.log('extractTextFromMessageContent: 未知的content类型:', typeof content);
    return '';
}

function extractJsonFromText(text) {
    if (!text) {
        console.log('extractJsonFromText: text is empty');
        return null;
    }
    
    console.log('extractJsonFromText: 原始文本:', text);
    
    // 尝试多种JSON匹配模式
    let match = text.match(/\{[\s\S]*\}/);
    if (!match) {
        console.log('extractJsonFromText: 未找到JSON格式内容');
        // 尝试查找可能的JSON片段
        const possibleJson = text.match(/```json\s*([\s\S]*?)\s*```/);
        if (possibleJson) {
            match = [possibleJson[1]];
            console.log('extractJsonFromText: 找到代码块中的JSON:', match[0]);
        }
    }
    
    if (!match) {
        console.log('extractJsonFromText: 仍然未找到JSON内容');
        return null;
    }
    
    try {
        const jsonStr = match[0];
        console.log('extractJsonFromText: 尝试解析JSON字符串:', jsonStr);
        ...
        console.log('extractJsonFromText: 清理后的JSON:', cleanedJson);
        
        const result = JSON.parse(cleanedJson);
        console.log('extractJsonFromText: 解析成功:', result);
        return result;
    } catch (e) {
        console.error('extractJsonFromText: JSON解析失败:', e);
        console.log('extractJsonFromText: 失败的JSON字符串:', match[0]);
        
        // 尝试修复常见的JSON格式问题
        try {
            let fixedJson = match[0]
                .replace(/([^\\])"/g, '$1"') // 修复可能的引号问题
                .replace(/,\s*}/g, '}') // 移除尾随逗号
                .replace(/,\s*]/g, ']'); // 移除数组尾随逗号
            
            console.log('extractJsonFromText: 尝试修复后的JSON:', fixedJson);
            const result = JSON.parse(fixedJson);
            console.log('extractJsonFromText: 修复后解析成功:', result);
            return result;
        } catch (e2) {
            console.error('extractJsonFromText: 修复后仍然解析失败:', e2);
            return null;
        }
    }
}
...
async function callTextLLM({ storyIdea, pageCount, advancedData }) {
    ...
    const userInstruction = [
        ...
        '',
        '请严格按照system prompt中的JSON结构输出，字段包含：title, theme, pages[{page, illustration_prompt, text}], ending。',
        '每一页都需要有清晰一致的人物设定与场景信息；文字简短生动，适合儿童。',
        '',
        '重要提示：',
        '1. 必须输出纯JSON格式，不要添加任何其他文字说明',
        '2. 确保JSON格式完全正确，可以被JSON.parse()解析',
        '3. 每个字段都要完整填写，不要遗漏',
        '4. 如果输出包含代码块，请将JSON放在```json```代码块中'
    ].join('\n');

    // 添加调试日志
    console.log('Text LLM API Key:', ARK_API_KEY);
    console.log('Text LLM Request URL:', `${ARK_BASE_URL}/chat/completions`);
    console.log('Text LLM Request headers:', {
    ...
    // 添加响应调试日志
    console.log('Text LLM Response status:', res.status);
    console.log('Text LLM Response headers:', Object.fromEntries(res.headers.entries()));

    if (!res.ok) {
        const errText = await res.text().catch(() => '');
        console.error('Text LLM Error response:', errText);
        throw new Error(`文案生成接口错误：${res.status} ${errText}`);
    }

    const data = await res.json();
    console.log('Text LLM Response data:', data);
    const contentText = extractTextFromMessageContent(data?.choices?.[0]?.message?.content);
    console.log('Text LLM Content text:', contentText);
    
    const json = extractJsonFromText(contentText);
    if (!json) {
        console.error('JSON解析失败，原始内容:', contentText);
        throw new Error('LLM返回的内容无法解析为有效的JSON格式。请检查返回内容格式。');
    }
    
    if (!Array.isArray(json.pages)) {
        console.error('JSON结构不正确，缺少pages数组:', json);
        throw new Error(`JSON结构不正确，缺少pages数组。当前JSON结构: ${JSON.stringify(json, null, 2)}`);
    }
    
    if (json.pages.length === 0) {
        console.error('pages数组为空:', json);
        throw new Error('生成的绘本页面数量为0，请检查LLM输出。');
    }
...
async function generateImageByPrompt(prompt) {
    // 添加调试日志
    console.log('API Key:', ARK_API_KEY);
    console.log('Request URL:', `${ARK_BASE_URL}/images/generations`);
